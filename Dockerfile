...
# We run the command in bash.
RUN bash -c "npm install --unsafe-perm --legacy-peer-deps --loglevel silly"
...
# Start the Node app (if needed, change to ["node", "/juice-shop/build/app.js"])
